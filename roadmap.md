...
**Infra bug found and fixed (blocked every magic link in production, not just invites):** Supabase's Auth **Site URL** was `http://localhost:3000` with an **empty redirect allow-list**. Supabase only honors the `emailRedirectTo` a request actually asks for when that URL matches the allow-list — with the list empty, every magic link (sign-in, sign-up, and invite acceptance alike) silently fell back to Site URL and sent people to localhost instead of the deployed app. Fixed directly via the Supabase Management API: `site_url` set to `https://shared-bills-tracker.vercel.app`, and `uri_allow_list` set to allow the production domain, Vercel preview deployments, and `localhost:3000` for local dev. Verified by generating a real magic link with `redirectTo` set to the production callback URL and confirming the returned link now actually points there instead of localhost. No app code changed — this was purely project configuration.

**Still needs a live test (can't be done headlessly):** actually clicking a generated invite link, completing sign-up via a real email inbox, and confirming the invitee lands on the dashboard with correct access — the magic-link email round trip needs a real inbox, same as the Phase 5 on-device install check. This should now actually be testable, since the redirect bug above was likely why it wasn't working before.
